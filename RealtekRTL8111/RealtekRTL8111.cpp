...
/* IONetworkController reserved methods for module support */
...
bool RTL8111::initRTL8111()
{
    ...
    IOLog("Initialized real MAC Address: ");
    ShowMacAddr(origMacAddr);

    if (emuMacAddrUsed == true)
    {
        if (&emuMacAddr.bytes)
        {
            IOLog("Setting hardware MAC address to the emulated value\n");
            setPromiscuousMode(true);
            setHardwareAddress(&emuMacAddr);
        }
    }

    IOLog("Ethernet [RealtekRTL8111]: detected %s (Chipset %d) at 0x%lx\n",
